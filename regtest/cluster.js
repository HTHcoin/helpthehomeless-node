...
describe('Helpthehomeless Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, process.env.HOME, './.helpthehomeless/data/helpthehomelessd')
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/helpthehomeless.conf'),
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/helpthehomeless.conf'),
      rpcuser: 'helpthehomeless',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/helpthehomeless.conf'),
      rpcuser: 'helpthehomeless',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 helpthehomelessd daemons');
    this.timeout(200000);
    async.each(nodesConf, function(nodeConf, next) {
      ...
      rimraf(path.resolve(nodeConf.datadir, './regtest'), function(err) {
        ...
        var client = new HelpthehomelessdRPC({
          protocol: 'http',
          host: '127.0.0.1',
  ...
  it('step 1: will connect to three helpthehomelessd daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = helpthehomeless.Networks.get('regtest');
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.helpthehomelessd.once('tip', function(height) {
