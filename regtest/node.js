...
var HelpthehomelessService = index.services.Helpthehomeless;
...
describe('Node Functionality', function() {
  ...
  before(function(done) {
    ...
    testKey = helpthehomeless.PrivateKey(testWIF);
    ...
    rimraf(datadir + '/regtest', function(err) {
      ...
      var configuration = {
        ...
        services: [
          {
            name: 'helpthehomelessd',
            module: HelpthehomelessService,
            config: {
              spawn: {
                datadir: datadir,
      ...
      node = new HelpthehomelessNode(configuration);

      regtest = helpthehomeless.Networks.get('regtest');
      ...
      node.start(function(err) {
        ...
        var syncedHandler = function() {
          if (node.services.helpthehomelessd.height === 150) {
            node.services.helpthehomelessd.removeListener('synced', syncedHandler);
            done();
          }
        };

        node.services.helpthehomelessd.on('synced', syncedHandler);
  ...
  describe('Bus Functionality', function() {
    it('subscribes and unsubscribes to an event on the bus', function(done) {
      ...
      bus.on('helpthehomelessd/hashblock', function(data) {
        bus.unsubscribe('helpthehomelessd/hashblock');
        if (blockExpected) {
  ...
  describe('Address Functionality', function() {
    ...
    before(function(done) {
      ...
      var startHeight = node.services.helpthehomelessd.height;
      node.services.helpthehomelessd.on('tip', function(height) {
    ...
    describe('History', function() {
      ...
      before(function(done) {
        ...
        testKey2 = helpthehomeless.PrivateKey.fromWIF('cNfF4jXiLHQnFRsxaJyr2YSGcmtNYvxQYSakNhuDGxpkSzAwn95x');
        address2 = testKey2.toAddress(regtest).toString();

        testKey3 = helpthehomeless.PrivateKey.fromWIF('cVTYQbaFNetiZcvxzXcVMin89uMLC43pEBMy2etgZHbPPxH5obYt');
        address3 = testKey3.toAddress(regtest).toString();

        testKey4 = helpthehomeless.PrivateKey.fromWIF('cPNQmfE31H2oCUFqaHpfSqjDibkt7XoT2vydLJLDHNTvcddCesGw');
        address4 = testKey4.toAddress(regtest).toString();

        testKey5 = helpthehomeless.PrivateKey.fromWIF('cVrzm9gCmnzwEVMGeCxY6xLVPdG3XWW97kwkFH3H3v722nb99QBF');
        address5 = testKey5.toAddress(regtest).toString();

        testKey6 = helpthehomeless.PrivateKey.fromWIF('cPfMesNR2gsQEK69a6xe7qE44CZEZavgMUak5hQ74XDgsRmmGBYF');
    ...
    describe('Mempool Index', function() {
      ...
      it('will update the mempool index after new tx', function(done) {
        var memAddress = helpthehomeless.PrivateKey().toAddress(node.network).toString();
        var tx = new Transaction();
        tx.from(unspentOutput);
        ...
        node.services.helpthehomelessd.sendTransaction(tx.serialize(), function(err, hash) {
