...
var helpthehomeless = require('@HTHcoin/helpthehomeless-lib');
var zmq = require('zeromq');
var async = require('async');
var LRU = require('lru-cache');
var HelpthehomelessdRPC = require('@HTHcoin/helpthehomelessd-rpc');
var $ = helpthehomeless.util.preconditions;
var _  = helpthehomeless.deps._;
var Transaction = helpthehomeless.Transaction;
var Proposal = helpthehomeless.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to helpthehomelessd in Node.js. Manages starting and
 * stopping helpthehomelessd as a child process for application support, as well as connecting
 * to multiple helpthehomelessd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Helpthehomeless(options) {
  if (!(this instanceof Helpthehomeless)) {
    return new Helpthehomeless(options);
  }
  ...
  // helpthehomelessd child process
  this.spawn = false;
  ...
  // available helpthehomelessd nodes
  this._initClients();
...
util.inherits(Helpthehomeless, Service);

Helpthehomeless.dependencies = [];

Helpthehomeless.DEFAULT_MAX_TXIDS = 1000;
Helpthehomeless.DEFAULT_MAX_HISTORY = 50;
Helpthehomeless.DEFAULT_MAX_UTXO = 1000;
Helpthehomeless.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Helpthehomeless.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Helpthehomeless.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Helpthehomeless.DEFAULT_SPAWN_RESTART_TIME = 5000;
Helpthehomeless.DEFAULT_SPAWN_STOP_TIME = 10000;
Helpthehomeless.DEFAULT_TRY_ALL_INTERVAL = 1000;
Helpthehomeless.DEFAULT_REINDEX_INTERVAL = 10000;
Helpthehomeless.DEFAULT_START_RETRY_INTERVAL = 5000;
Helpthehomeless.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Helpthehomeless.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Helpthehomeless.DEFAULT_INSTANTSEND_FEE = 10000;
Helpthehomeless.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'helpthehomeless',
  rpcpassword: 'local321',
  uacomment: 'helpthehomeless'
};

Helpthehomeless.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Helpthehomeless.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Helpthehomeless.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Helpthehomeless.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Helpthehomeless.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Helpthehomeless.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Helpthehomeless.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Helpthehomeless.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Helpthehomeless.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Helpthehomeless.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Helpthehomeless.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Helpthehomeless.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Helpthehomeless.prototype._initCaches = function() {
...
Helpthehomeless.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Helpthehomeless.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Helpthehomeless.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'helpthehomelessd/transactionlock',
      scope: this,
    ...
    {
      name: 'helpthehomelessd/addresstxid',
      scope: this,
...
Helpthehomeless.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'helpthehomelessd/' + name, 'total:', this.subscriptions[name].length);
};

Helpthehomeless.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'helpthehomelessd/' + name, 'total:', this.subscriptions[name].length);
};

Helpthehomeless.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (helpthehomeless.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'helpthehomelessd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Helpthehomeless.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'helpthehomelessd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Helpthehomeless.prototype.unsubscribeAddressAll = function(emitter) {
...
Helpthehomeless.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Helpthehomeless.DEFAULT_CONFIG_SETTINGS;
...
Helpthehomeless.prototype._parseHelpthehomelessConf = function(configPath) {
  var options = {};
...
Helpthehomeless.prototype._expandRelativeDatadir = function() {
...
Helpthehomeless.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in helpthehomelessd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in helpthehomelessd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in helpthehomelessd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './helpthehomeless.conf');

  log.info('Using Helpthehomeless config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseHelpthehomelessConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseHelpthehomelessConf(networkConfigPath));
  }
...
Helpthehomeless.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of helpthehomeless-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of helpthehomeless-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of helpthehomeless-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to helpthehomelessd from helpthehomeless. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from helpthehomelessd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
      spawnConfig.zmqpubrawtxlock,
      '"zmqpubrawtxlock" option is required to get transaction locks from helpthehomelessd. ' +
      'Please add "zmqpubrawtxlock=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from helpthehomelessd. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in helpthehomeless.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that helpthehomelessd is undergoing a reindex. ' +
             ...
             'of helpthehomeless-node services will start.');
    node._reindex = true;
  }
};

Helpthehomeless.prototype._resetCaches = function() {
 ...
 * @private
 */
Helpthehomeless.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Helpthehomeless.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Helpthehomeless.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('Helpthehomeless Daemon Ready');
          callback();
        });
      });
...
Helpthehomeless.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === helpthehomeless.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Helpthehomeless.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === helpthehomeless.Networks.testnet) {
    networkPath = 'testnet3/helpthehomeless.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/helpthehomeless.conf';
    }
  }
  return networkPath;
};

Helpthehomeless.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === helpthehomeless.Networks.testnet) {
...
Helpthehomeless.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('helpthehomelessd/hashblock', message.toString('hex'));
    }
  }

};

Helpthehomeless.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Helpthehomeless.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Helpthehomeless Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Helpthehomeless.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Helpthehomeless.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('helpthehomelessd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Helpthehomeless.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = helpthehomeless.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('helpthehomelessd/rawtransaction', message.toString('hex'));
    }

    var tx = helpthehomeless.Transaction();
    tx.fromString(message);
    var txid = helpthehomeless.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Helpthehomeless.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = helpthehomeless.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('helpthehomelessd/transactionlock', message.toString('hex'));
    }

  }
};

Helpthehomeless.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Helpthehomeless.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Helpthehomeless.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Helpthehomeless.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Helpthehomeless.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Helpthehomeless Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Helpthehomeless.DEFAULT_REINDEX_INTERVAL);

};

Helpthehomeless.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Helpthehomeless.prototype._stopSpawnedHelpthehomeless = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/helpthehomelessd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned helpthehomeless process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean helpthehomeless process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Helpthehomeless.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedHelpthehomeless(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting helpthehomeless process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Helpthehomeless process unexpectedly exited with code:', code);
        log.warn('Restarting helpthehomeless child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Helpthehomeless process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new HelpthehomelessdRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn helpthehomelessd.'));
      }
...
Helpthehomeless.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to helpthehomelessd.'));
    }
 ...
 */
Helpthehomeless.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Helpthehomeless configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.syncPercentage = function(callback) {
  var self = this;
...
Helpthehomeless.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, helpthehomeless address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = helpthehomeless.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Helpthehomeless.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Helpthehomeless.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Helpthehomeless.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in helpthehomeless is most recent to less recent
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Helpthehomeless.prototype._getConfirmationsDetail = function(transaction) {
...
Helpthehomeless.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Helpthehomeless.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Helpthehomeless.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A helpthehomeless transaction id
 * @param {Function} callback
 */
Helpthehomeless.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Helpthehomeless.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof helpthehomeless.Address) {
...
Helpthehomeless.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Helpthehomeless.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Helpthehomeless object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Helpthehomeless.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = helpthehomeless.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Helpthehomeless.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Helpthehomeless.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Helpthehomeless Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Helpthehomeless.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Helpthehomeless.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Helpthehomeless.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Helpthehomeless.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Helpthehomeless.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Helpthehomeless.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Helpthehomeless.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Helpthehomeless.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Helpthehomeless.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Helpthehomeless.prototype.govCount = function (callback) {
  var self = this;
...
Helpthehomeless.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Helpthehomeless.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Helpthehomeless.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Helpthehomeless.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.getInfo = function(callback) {
  var self = this;
...
Helpthehomeless.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Helpthehomeless.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('helpthehomelessd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('helpthehomelessd process did not exit'));
      }
...
module.exports = Helpthehomeless;
