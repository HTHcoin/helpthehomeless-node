...
/**
 * Will return the path and default helpthehomeless-node configuration. It will search for the
 * configuration file in the "~/.helpthehomeless" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.helpthehomeless');
  var defaultConfigFile = path.resolve(defaultPath, './helpthehomeless-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['helpthehomelessd', 'web'];
