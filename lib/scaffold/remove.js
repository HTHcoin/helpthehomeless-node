...
/**
 * Will remove a service from helpthehomeless-node.json
...
/**
 * Will remove the Node.js service and from the helpthehomeless-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The helpthehomeless-node configuration directory
...
function remove(options, done) {
  ...
  var helpthehomelessConfigPath = path.resolve(configPath, 'helpthehomeless-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(helpthehomelessConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a helpthehomeless-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to helpthehomeless-node.json
        removeConfig(helpthehomelessConfigPath, service, next);
      });
    }, done
  );
}
