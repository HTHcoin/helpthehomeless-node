...
var HelpthehomelessNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for helpthehomelessd is necessary for this upgrade with the "reindex=1" helpthehomeless.conf option. \n' +
                 'There are changes necessary in both helpthehomeless.conf and helpthehomeless-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/Altcoin-Cash/helpthehomeless-node/blob/helpthehomelessd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          helpthehomelessd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/helpthehomelessd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed helpthehomeless-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './helpthehomeless-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in helpthehomeless-node services directory
    ...
    if (servicePackage.helpthehomelessNode) {
      serviceModule = service.name + '/' + servicePackage.helpthehomelessNode;
    }
 ...
 * @param {Node} node - The Helpthehomeless Node instance
 */
