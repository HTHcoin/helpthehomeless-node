...
var BASE_PACKAGE = {
  description: 'A full Helpthehomeless node build with Helpthehomeless',
  repository: 'https://github.com/Altcoin-Cash/helpthehomeless-node',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    // '@Altcoin-Cash/helpthehomeless-lib': '^' + helpthehomeless.version,
    // '@Altcoin-Cash/helpthehomeless-node': version
    'helpthehomeless-lib': 'Altcoin-Cash/helpthehomeless-lib#master',
    'helpthehomeless-node': 'Altcoin-Cash/helpthehomeless-node',
  }
};

/**
 * Will create a directory and helpthehomeless.conf file for Helpthehomeless.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createHelpthehomelessDirectory(datadir, done) {
...
/**
 * Will create a base Helpthehomeless Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The helpthehomeless database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/helpthehomeless-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Helpthehomeless Node directory, configuration file,
 * helpthehomeless.configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the helpthehomeless node configuration directory
 * @param {String} options.datadir - The path to the helpthehomeless datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the helpthehomeless-node directory and configuration
    ...
    function(next) {
      // Setup the Helpthehomeless directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createHelpthehomelessDirectory(absDataDir, next);
      } else {
        next();
      }
    },
